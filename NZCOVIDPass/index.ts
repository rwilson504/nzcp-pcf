...
export class NZCOVIDPass implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	private _nzcpResult: nzcp.VerificationResult;
	...
	{
		let options = context.parameters.testMode.raw ? 
		{
			trustedIssuer: nzcp.TRUSTED_ISSUERS.MOH_EXAMPLE,
			didDocument: nzcp.DID_DOCUMENTS.MOH_EXAMPLE
		} :
		{};

		this._nzcpResult = context.parameters.offlineMode.raw ?
			nzcp.verifyPassURIOffline(context.parameters.QRCode.raw || '', options) :
			await nzcp.verifyPassURI(context.parameters.QRCode.raw || '', options);
